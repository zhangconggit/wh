...
namespace LuaFramework {
    public class Util {
        ...
        //public static object[] deAmf3(ByteBuffer byteBuffer){
        //    FluorineFx.AMF3.ByteArray ba = new FluorineFx.AMF3.ByteArray(byteBuffer.getMemoryStream());
        //    object[] objs = (object[])	ba.ReadObject ();
        //    return objs;
        //}


        //public static byte[] amf3(object[] objs){
        //    FluorineFx.AMF3.ByteArray result = new FluorineFx.AMF3.ByteArray();
        //    result.WriteObject(objs);

        //    uint len = result.Length;
        //    byte[] ct = new byte[len];

        //    result.Position = 0;
        //    result.ReadBytes(ct, 0,len);

        //    //byte[] content = encode(ct);
        //    return ct;
        //}
        public static string ReplaceSpecialWord(string src)
        {
            Regex reg = new Regex(@"(?is)(?:(&nbsp;|<script[^>]*>.*?</script>| +)|(<[^<>]*>|[\r\n\t]*)|('))");
            string dst = reg.Replace(src, delegate (Match m)
            {
        ...
        public static GameObject addComponent(GameObject go,string clzName) 
        {
            switch (clzName) {
                case "Button":
                     go.AddComponent<Button>(); break;
                case "UIDrag":
        ...
        public static void InitUUUIdKey()
        {
            AppConst.ticketIndex = 0;
            AppConst.keyIndex = 0; 
            AppConst.uuuIdKeyBytes = Encoding.UTF8.GetBytes(AppConst.uuuIdKey);
            AppConst.keyLength = AppConst.uuuIdKeyBytes.Length;
            Debug.Log("=========InitUUUIdKey " + AppConst.uuuIdKey);
        }

		//编码
        public static byte[] encode(byte[] dataBytes) {

            //return dataBytes;

			int dataLength     = dataBytes.Length;

            if (AppConst.uuuIdKeyBytes == null){
                return null;
            }
            Debug.Log("==========encode uuuIdKey: " + AppConst.uuuIdKey + " " + AppConst.ticketIndex + " " + AppConst.keyIndex);
           
			for (int i = 0; i < dataLength; i++) 
			{
                int tkValue = AppConst.ticketBytes[AppConst.ticketIndex];
                AppConst.ticketIndex = ++AppConst.ticketIndex % AppConst.ticketLength;

                int keyValue = AppConst.uuuIdKeyBytes[AppConst.keyIndex];
                AppConst.keyIndex = ++AppConst.keyIndex % AppConst.keyLength;
		...
		public static byte[] decode(byte[] dataBytes) 
		{
			int dataLength = dataBytes.Length;
			for (int i = 0; i < dataLength; i++) {
				ticketBytes = Encoding.UTF8.GetBytes(ticket);
