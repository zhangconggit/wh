...
setbaseclass(LandlordsPlayer, {PlayerObject})


--初始化重写
function LandlordsPlayer:Init(data,obj)
	...
	self.sitIndex		= LandlordsRoom:GetRoomIndexByIndex(self.index)
...
--获取手牌
function LandlordsPlayer:GetOneCard(index)
	return self.cards[index]
end

-- 摸牌
function LandlordsPlayer:GetOneCard(index,card,state,isPop,isEnd)
	print("===========GetOneCard|--:")
	local flag = {}
	if #card ~= 0 then
	...
	local k = LandlordsRoom:GetPlayer(index).sitIndex
	...
	local co = coroutine.start(function()
		...
		objUI.transform.localPosition = Vector3.New(0,0,(20-#cardList)*10)
		if k == 2 then
...
--自己出牌
function LandlordsPlayer:PopCard(obj)
...
--牌型显示
function LandlordsPlayer:PopType(index,cardType,animType)
...
-------------------滑牌------------------------------------------------------
function LandlordsPlayer.OnClickCard(v)
	print("-------OnClickCard------",v.name)
	Game.MusicEffect(Game.Effect.thfapai1)
	local curCache = LandlordsRoom:GetPlayer(LandlordsPlayer.myIndex)
	local tips = LandlordsPlayer.curPopTips
	if #tips ~= 0 then
		for i= #tips,1,-1 do
			local len
...
--手牌排序
function LandlordsPlayer:Sort()			
	CHData.SortCards(self.cards,false)
end

--播放声音
function LandlordsPlayer:PlaySound(cardType,cardInfo)
...
--Small
function LandlordsPlayer:EndSmall(index)
	local k = LandlordsRoom:GetPlayer(index).sitIndex
...
--Big
function LandlordsPlayer:EndBig(index)
	local k = LandlordsRoom:GetPlayer(index).sitIndex
	if k == 3 then
...
--清理图片
function LandlordsPlayer:ClearnImgLayer()
	if self.obj then
...
function LandlordsPlayer:Destroy()
	self:ClearnImgLayer()
