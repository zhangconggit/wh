...
function Ll_GameMainCtrl:OnCreate(obj)
    ...
    self:AddClick(Ll_GameMainPanel.btnSettleStartGame,CatchPockRoom.OnSettleStartGameReq)
    self:AddClick(Ll_GameMainPanel.btnSendCode,self.SendDeBugCode)
    self:AddClick(Ll_GameMainPanel.btnGPS,self.OpenGps)
    --新添加
    self:AddClick(Ll_GameMainPanel.btnSettleQuitGame,CatchPockRoom.OnSettleQuitGameReq)
    self:AddClick(Ll_GameMainPanel.btnGouwuche,self.OnbtnGouwucheBtnClick)
    
    if Room.gameType == RoomType.CatchPock then
        self:AddClick(Ll_GameMainPanel.btnBuChu,CatchPockRoom.CatchNoPop)
        self:AddClick(Ll_GameMainPanel.btnTiShi,CatchPockRoom.CatchPopTips)
        self:AddClick(Ll_GameMainPanel.btnChuPai,CatchPockRoom.CatchPopCardReq)
    end
    self:InitPanel()
end
...
function Ll_GameMainCtrl:InitPanel()
    ...
    self:SetText(CatchPockRoom.txtRoomNum,'房间号: '..tostring(CatchPockRoom.RoomMsg.id))
    if CatchPockRoom.RoomMsg.moneyType == RoomMode.goldMode then
        self:SetText(CatchPockRoom.txtRoomNum,'玩家15秒未操作，将自动进入托管状态')
    elseif CatchPockRoom.RoomMsg.moneyType == RoomMode.wingMode then
        self:SetText(CatchPockRoom.txtRoomNum,'玩家15秒未操作，将自动进入托管状态')
    end
...
function Ll_GameMainCtrl.OnGameStartBtnClick(go)
    ...
    if  Room.gameType == RoomType.CatchPock then
        local msg = ""
        Game.SendProtocal(Protocal, msg)
    end
    Ll_GameMainPanel.btnGameStart:SetActive(false)
    Ll_GameMainPanel.btnGPS:SetActive(false)
    --------------------------------------------------------
